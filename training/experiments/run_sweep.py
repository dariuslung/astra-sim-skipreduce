...
def main():
    ...
    for i, cfg in enumerate(configs_to_run):
        ...
        cmd = [
            args.python_bin, "training/experiments/train_cifar.py",
            "--ranks", str(cfg["ranks"]),
            "--skip", str(cfg["skip"]),
        ...
        # Find the latest generated log file in training/logs
        log_files = sorted(
            [os.path.join("training/logs", f) for f in os.listdir("training/logs") if f.endswith(".json")],
